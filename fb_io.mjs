...
export {
    ...
    infoRegistration, fb_writeAuth, fb_read_sorted, leaderboardRead, showNecessary,
    // checkUID,
...
function fb_onAuthStateChanged() {
    ...
    onAuthStateChanged(AUTH, (user) => {
        ...
        userUID = user.uid;
        sessionStorage.setItem("UID", userUID)
        console.log("fb_onAuthStateChanged second time is running");
        const dbReference = ref(fb_gamedb, "details/users/" + userUID);
        get(dbReference).then((snapshot) => {
            if (!snapshot.exists() || !snapshot.child("Name").exists()) {
                window.location.replace("registration/registration.html");
                console.log("fb_onAuthStateChanged 3rd time is running");
                return;
            }
        }).catch((error) => {
            console.log(error);
...
function infoRegistration() {
    ...
    console.log({ age, name, gender, pasta, phoneNumber, address, wonder });
...
async function joinLobby() {
    ...
    get(dbReference).then(async (snapshot) => {
        ...
        if (fb_data != null) {
            ...
            console.log(player2);
            console.log("This is your name test", name);
